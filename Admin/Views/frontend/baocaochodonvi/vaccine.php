...
<body>
    ...
    <nav class="navbar navbar-light justify-content-between" style="background-color: #e3f2fd">
        ...
        <a href="./index.php?controller=thongketheodonvi&action=dotuoi" ><button class="btn btn-outline-success">Thống kê độ tuổi</button></a>
        <a href="./index.php?controller=thongketheodonvi&action=vaccine" ><button class="btn btn-danger">Thống kê vaccine</button></a>
        <form method="post" action="./index.php?controller=thongketheodonvi&action=vaccine" class="form-inline">
    ...
    <div class="container-fluid">
        <form method="post" action="./index.php?controller=thongketheodonvi&action=vaccine" class="form-inline mt-3">
        ...
        <div id="table" class="tableFixHead table-responsive mt-3" style="<?php if(empty($tendonvi)){echo"display:none";} ?>">
            <H4 style="text-align:center; padding:10px 0 30px 0; color:#4263f5">THỐNG KÊ SỐ LƯỢNG NGƯỜI DÂN THEO MŨI TIÊM</H4>
            <div class="container">
                ...
                <table id="tableExcel" class="table table-bordered mt-2">
                    ...
                    <tbody class="row-table">
                        <?php 
                            $sum = 0;
                            $lastIndex = count($vaccines) + 1 ;
                            for($i = 0; $i<count($vaccines); $i++){
                                $stt = $i +1;
                                $sum += $soluongs[$i]['soluong'];
                                echo "
                                    <tr class='row-content'>
                                        <td>$stt</td>
                                        <td>".$vaccines[$i]["TenVacxin"]."</td>
                                        <td>".$soluongs[$i]['soluong']."</td>
                                    </tr>";
                                }
                                echo "
    ...
    <script src="./Views/frontend/public/table2excel/dist/table2excel.js"></script>
